...
// Taken from https://github.com/libgit2/libgit2sharp/blob/master/LibGit2Sharp.Tests/TestHelpers/DirectoryHelper.cs
        ...
        private static void DeleteDirectory(string directoryPath, int maxAttempts, int initialTimeout, int timeoutFactor)
        {
            for (int attempt = 1; attempt <= maxAttempts; attempt++)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Trace.WriteLine(
                        string.Format(
                            "{0}The directory '{1}' could not be deleted ({2} attempts were made) due to a {3}: {4}" +
                            ...
                            Environment.NewLine, Path.GetFullPath(directoryPath), maxAttempts, caughtExceptionType,
                            ex.Message));
                }
            }
        }
